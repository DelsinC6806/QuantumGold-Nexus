...
COMMISSION_PER_LOT = 7.00    # Adjust to your broker's round-turn commission
...
def backtest_time_based_single(data, initial_balance=100000):
    ...
    df_temp['sma'] = df_temp['close'].rolling(window=20).mean()
    
    atrs = df_temp['atr'].values
    sigmas = df_temp['sigma'].values
    smas = df_temp['sma'].values
    ...
    for i in range(50, len(data)):
        ...
        if position is not None:
            ...
            # Intentionally broken BE trigger distance (per user request)
            ...
            if exit_price is not None:
                ...
                if exit_reason == "WIN":
                    ...
                    pnl = gross_pnl - commission_cost # gross_pnl is already negative here
                else:
                    pnl = -commission_cost # Scratches still pay commission
        ...
        if current_ts_is_time(ts, hour=20, minute=0) and position is None:
            ...
            if np.isnan(atr_val) or np.isnan(sigma_val) or np.isnan(sma_val):
                continue
            
            current_price = curr_candle['open']

            # --- MEAN EXTENSION (RUBBER BAND) FILTER ---
            # Do not enter if price is already stretched > 1.5 standard deviations from the 20 SMA
            if signal == 1 and current_price > (sma_val + (sigma_val * 1.5)):
                continue 
            if signal == -1 and current_price < (sma_val - (sigma_val * 1.5)):
                continue 
            # -------------------------------------------
            ...
            # Fix Lookahead Bias: Execute on OPEN, pay spread on entry
            if signal == 1:  # BUY
