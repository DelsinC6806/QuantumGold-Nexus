...
# --- NEW: Max duration cap to avoid overnight trades ---
...
def backtest_time_based_single(data, initial_balance=100000):
    ...
    for i in range(50, len(data)):
        ...
        if position is not None:
            ...
            # --- FIX: check duration every 15 minutes ---
            # Instead of only checking once per candle, we simulate 15m checks
            # by comparing elapsed time and forcing exit if exceeded
            if ts - position['entry_ts'] >= MAX_DURATION:
                exit_price = curr_candle['open']  # exit at next candle open
            ...
            if exit_price is not None:
                ...
                pnl = gross_pnl - commission_cost if exit_reason in ["WIN","LOSS","TIME_EXIT"] else -commission_cost
    ...
    time_exits = [t for t in trades if t['reason'] == 'TIME_EXIT']
    
    win_trades = len(wins)
    lose_trades = len(losses)
    scratch_trades = len(scratches)
    time_exit_trades = len(time_exits)
    ...
    print(f"Time Exits:               {time_exit_trades} ({(time_exit_trades / total_trades) * 100:.1f}%)")
...
if __name__ == "__main__":
    try:
        ...
        print("Error: 'history_data.csv' not found")