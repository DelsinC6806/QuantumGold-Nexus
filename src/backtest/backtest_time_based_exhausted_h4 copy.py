...
# === Exhaustion Filter Config ===
# Skips trade if the trigger H4 candle range is > 1.75x the current ATR
...
def backtest_time_based_single(data, initial_balance=50000):
    ...
    # Pre-calculate high/low rolling slices to precisely evaluate the structural H4 candle range
    ...
    for i in range(50, len(data)):
        ...
        if current_ts_is_time(ts, hour=20, minute=0) and position is None:
            ...
            # --- Exhausted Candle Evaluation ---
            # Reconstruct the highest high and lowest low across the 16-period H4 horizon
            h4_candle_high = np.max(highs[i-16:i])
            h4_candle_low = np.min(lows[i-16:i])
            h4_total_range = h4_candle_high - h4_candle_low
            
            # If the block's movement expands way beyond current normal distribution boundaries, step aside
            if h4_total_range > (atr_val * EXHAUSTION_THRESHOLD):
                continue
